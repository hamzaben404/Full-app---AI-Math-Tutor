...
export default function QuizReview({ questions, userAnswers }: QuizReviewProps) {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-3xl font-bold">Quiz Review</CardTitle>
      </CardHeader>
      <CardContent>
          {questions.map((question, questionIndex) => (
            <div key={questionIndex} className="mb-8 last:mb-0">
              ...
              {/* --- AJOUT DU BLOC JUSTIFICATION --- */}
              <div className="mt-4 p-3 rounded-lg bg-muted/50 border border-muted-foreground/20">
                <p className="font-semibold text-sm mb-1">Justification :</p>
                <p className="text-sm text-muted-foreground">{question.justification}</p>
              </div>
              {/* --- FIN DU BLOC --- */}
            </div>
          ))}
